...
const pages = ['Acasă','CV', 'Joburi'];
const settings = ['Profil','Joburi Salvate','Aplicațiile mele','Deconectează-te'];
...
const ClientNavBar = () => {
    ...
    useEffect(()=>{
        ChatService.getConversations()
            .then(result=>{
                if(result.data.some(element=>element.read===false)){
    ...
    const handleRedirectUserMenu = (setting)=>{
        if(setting==="Profil"){
            ...
            if(setting==='Deconectează-te'){
                ...
                if(setting==='Joburi Salvate'){
                    history.push("/favorites");
                }
                else{
                    if(setting==='Aplicațiile mele'){
